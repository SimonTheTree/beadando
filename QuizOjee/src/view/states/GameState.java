...
import view.components.AdPopup;
...
public class GameState extends State implements GameInputListener {
	...
	public void calcNeighbors() {
		Set<Territory> own = new TreeSet<>(player.getTerritories());
...
//		infoPanel.setBounds(0, settings.GAME_HEIGHT, settings.SCREEN_WIDTH ,settings.SCREEN_HEIGHT);
	...
	public void displayRaceQuestionGUI(RaceQuestion quest, String attacker, String defender){
	...
	public void displayRaceQuestionBystander(RaceQuestion quest, String attacker, String defender){
		rqDialog.setInputEnabled(false);
		displayRaceQuestionGUI(quest, attacker, defender);
	...
	public void render() {
		if(!gameStarted) {
			g.drawImage(Resources.GAME_WAIT_FOR_JOIN_BG, 0, 0, getWidth(), getHeight(), this);
			return;
		}
		if (ticks % 15 == 1)
			redraw();
...
//			}
			if (inputManager.isClicked("ButtonLeft") && !gameOver) {
				...
				if (lit.equals(Territory.NULL_TERRITORY)) return; //ezt sehogyse tamadjuk! (mert ugye ez van begyújtva, ha más nincsen)
...
//					} else if(toBeLit.getOwner().getUser().getUsername().equals(uname)) {
//						valid = false; //magunkat ne tamadjuk
//					} else if(!neighbors.contains(toBeLit)) {
//						valid = false; //ne tamadjunk olyat aki nem szomszed
//					}
				
					if(neighbors.contains(toBeLit)) {
...
//						gameboard.setHighlitCell(gameboard.fromPixel(-10, -10));
	...
	public void manageNewOwner(GameMessage msg) {
		...
		Player newOwner = settings.getPlayerByUname(msg.getParams()[0]);
		int terrId = Integer.parseInt(msg.getParams()[1]);
		Territory terr = gameboard.getTerrytoryById(terrId);
		System.out.println("before new owner: "+terr);
		terr.setOwner(newOwner);
		System.out.println("after new owner: "+terr);
		calcNeighbors();
		updateInfoPanel();
	}
	...
	public void manageRaceQuestion(GameMessage msg) {
		...
		if(isItMine){
			...
			displayRaceQuestionBystander(question, attackerUname, defenderUname);
		}
	}
	...
	public void manageGameboard(GameMessage msg) {
		...
		//fix territory owner pointers:
		for(Territory t : gameboard.territories) {
			t.setOwner(settings.getPlayerByUname(t.getOwner().getUser().getUsername()));
		}
		calcNeighbors();
	}
	public void manageSettings(GameMessage msg) {
		GameSettings.setInstance((GameSettings) StringSerializer.deSerialize(msg.getParams()[0]));		
